...
double BinomialEuropeanPutOption::calculateExpectation(
    const PricingModelParameters& parameters,
    const ModelInternalParameters& internalParams) {

    double option_price = 0;

    for (int i = 0; i < internalParams.cRRThresholdIndex; ++i) {
        ...
        const double payoff_value = parameters.strike_price - expiry_share_price;
